...
def index(req):
    if req.method == 'GET':
        return render_as_env(req, 'account/supplier/index.html')
    try:
        method = req.POST['method']
        data = extract_data(req.POST)
        if method == 'read':
            accounts = [transform_item(item) for item in Person.objects.filter(person_role=False)]
            return response_as_json(accounts)
        elif method == "update":
            account = Person.objects.get(pk=data['id'])
            form = SupplierForm(data, instance=account)
...
def get(req):
    suppliers = [{
        'value': item.id,
        'label': item.name
        } for item in Person.objects.filter(person_role=False)]
    return JsonResponse({
        'data': suppliers,
    })

urls = [
    path("account/supplier", index, name="account.supplier.index"),
    path("account/suppliers", get, name="account.supplier.get"),
]